<?php
...

    {
        ...
        public function kullaniciTipleriOlustur() {
            $user = Sentinel::getUser();
            $sirketID = $user->sirket_id;

            $kullaniciTipiAdi = "Şantiye Şefi";
            $kullaniciTipiSlug = Str::slug($kullaniciTipiAdi, '-');

            $role = $this->createRole($kullaniciTipiAdi, $kullaniciTipiSlug);
            if($role) {

                $kullaniciTip = new KullaniciTipleri;
                $kullaniciTip->adi = $kullaniciTipiAdi;
                $kullaniciTip->sirket_id = $sirketID;
                $kullaniciTip->olusturan_id = $user->id;
                $kullaniciTip->role_id = $role->id;
                $kullaniciTip->save();

                return redirect()
                    ->back()
                    ->with('success', 'Kullanıcı tipi başarıyla eklendi.');
            } else {
                return redirect()
                    ->back()
                    ->with('warning', 'Kullanıcı tipi eklenemedi.');
            }
        }
        ...
        public function createRole($name, $slug) {
            ...
            if(! $role) {
                ...
                $role->permissions = [
                    ...
                    'santiye.santiyeGiderleri.goruntule' => true,
                    'santiye.santiyeGiderleri.ekle' => true,
                    'santiye.santiyeGiderleri.duzenle' => true,
                    'santiye.santiyeGiderleri.sil' => true,
                    'santiye.calisanMaaslari.odemeTalebiOlustur' => true,
                    'santiye.calisanMaaslari.goruntule' => true,
