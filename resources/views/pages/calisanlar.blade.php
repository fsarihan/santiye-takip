	...
	<div class="card card-custom">
		...
		<div class="card-body">
			...
			<table class="table table-bordered table-hover" id="kt_datatable">
				...
				@foreach($data['calisanlar'] as $calisan)
					<tr>
						<td>{{$calisan->id}}</td>
						<td>{{$calisan->adi}}</td>
						<td>{{$calisan->telefon}}</td>
						<td>{{$calisan->meslek_kodu}}</td>
						<td>Çalışma Durumu</td>
						<td>
							@foreach($calisan->santiyeler as $santiye)
								{{$santiye[0]}},
							@endforeach
						</td>
						<td>
							@foreach($calisan->uzmanlik as $uzmanlik)
								{{$uzmanlik}},
							@endforeach
						</td>
						{{--<td>{{$calisan->uzmanlik}}</td>--}}
						<td>{{$calisan->created_at}}</td>
						<td>xx</td>
						<td nowrap></td>
					</tr>
